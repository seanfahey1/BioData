...
facet_wrap(
  ~ Garden.location, # different plots for location
  ncol = 1
  ) +
theme_bw()
```
...
```{r t-test}
t.test(Ozone ~ Garden.location, data = ozone) # y ~ x
```
...
```{r combine into DF}
new.data <- data.frame(new.x, new.y) # combine test data and predictions into new dataset
new.data <- rename(new.data, EGGS = fit) # rename columns
new.data
```
